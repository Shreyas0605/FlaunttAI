# Lekha (ಲೇಖ) — English Helper Keyboard (Gemini-powered)
...
runs → download the **lekha-debug-apk** artifact → install `app-debug.apk` on the phone.

```bash
git init && git add . && git commit -m "Lekha Gemini"
git branch -M main
...
1. Open **Lekha** → **Enable Lekha Keyboard** → turn it on → **Switch to Lekha**.
2. Get a free Gemini key: go to **aistudio.google.com**, sign in, **Get API key**, create one
   (starts with `AIza…`).
3. In Lekha, paste the key and tap **Test & save**. On success it's stored.
...
- To use a newer model, change one line: `MODEL` in `app/src/main/java/com/lekha/keyboard/ai/Rewriter.kt`
...
## Structure
```
app/src/main/java/com/lekha/keyboard/
  MainActivity.kt              onboarding + Gemini API key setup
  ime/LekhaImeService.kt       keyboard + green suggestions + ✨ rewrite panel
